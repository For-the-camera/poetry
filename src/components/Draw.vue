<template>
    <div class="layout">
        <div class="header">
            <div class="nav">
                <el-button v-for="tool in tools" :key="tool.name" @click="drawType(tool)" :leftIcon="tool.icon"
                    :class="{ 'selected': tool.ischoose }">{{ tool.name }}
                </el-button>
                <el-button v-for="tab in tabs" :key="tab.name" :label="tab.name" class="tab demo-flat-button"
                    :icon="tab.icon" @click="tabfun(tab.fun)" primary backgroundColor="#FFFFFF">{{ tab.name }}</el-button>
                <el-button :label="chooseColorBtn" class="tab demo-flat-button" icon="color_lens" @click="setColor()"
                    primary backgroundColor="#FFFFFF" :color="color.hex">选择颜色</el-button>
                <Sketch v-model="color" v-if="ischoosecolor" style="position:absolute;z-index:10;top:56px;" @ok="setColor()"
                    @cancel="falseColor()" />
                <div class="penColor" :style="{ backgroundColor: color.hex }" @click="setColor()">
                </div>
